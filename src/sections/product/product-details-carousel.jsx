...
export default function ProductDetailsCarousel({ product }) {
  ...
  const slides = product?.pictures?.map((img) => ({
    src: img,
  }));
  ...
  const renderLargeImg = (
    ...
    >
      ...
      >
        {slides?.map((slide) => (
          <Image
            key={slide?.src?.image_url}
            alt={slide?.src?.image_url}
            src={slide?.src?.image_url}
            ratio="1/1"
            onClick={() => lightbox.onOpen(slide?.src?.image_url)}
            sx={{ cursor: 'zoom-in' }}
          />
        ))}
      </Carousel>
      ...
      >
        {slides?.map((item, index) => (
          <Box key={item.src} sx={{ px: 0.5 }}>
            <Avatar
              key={item?.src?.image_url}
              alt={item?.src?.image_url}
              src={item?.src?.image_url}
              variant="rounded"
              sx={{
                width: THUMB_SIZE,
                height: THUMB_SIZE,
